...
as `#1 - 7 citas`, plus compact links to the posts that quoted them. Every
rendered post also gets a clickable page badge, such as `Pag. 1`, so its
original thread page stays visible after sorting and can be filtered directly.
...
- `[` and `]` jump to the previous or next visible cited post.
